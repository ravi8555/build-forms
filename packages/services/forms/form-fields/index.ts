...
//       .values({ label, labelKey, type, formId, description, placeholder, isRequired: isRequired ?? false,
//         index })
...
//     const result = await db.select().from(formFieldsTable).where(eq(formFieldsTable.id, fieldId));
